...
pub fn parse_iso_date(value: &str) -> Option<i64> {
	...
	// Microseconds and a numeric offset need SSSSSS/XXX, which not every date
	// parser implements, so split the timestamp by hand: drop the fractional
	// seconds, apply the trailing offset ourselves, and parse what is left as a
	// naive UTC datetime. The unquoted "T" is the same last-resort fallback
	// en.novelbuddy uses, for parsers without Unicode quoted literals.
	let (head, offset) = split_utc_offset(value)?;
	let mut parts = head.splitn(2, '.');
	let naive = parts.next()?;
	// Anything left after the seconds must be the fractional part and nothing
	// else, or a trailing unparsed zone designator would be silently dropped.
	if let Some(fraction) = parts.next()
		&& (fraction.is_empty() || !fraction.bytes().all(|b| b.is_ascii_digit()))
	{
		return None;
	}
	let seconds = parse_date(naive, "yyyy-MM-dd'T'HH:mm:ss")
		.or_else(|| parse_date(naive, "yyyy-MM-ddTHH:mm:ss"))?;
	Some(seconds - offset)
}

/// Splits a trailing zone designator off an ISO 8601 timestamp, returning the
/// rest of the string and the offset from UTC in seconds. A bare timestamp is
/// read as UTC; a malformed offset is rejected outright.
fn split_utc_offset(value: &str) -> Option<(&str, i64)> {
	if let Some(head) = value.strip_suffix('Z') {
		return Some((head, 0));
	}
	let time_start = value.find('T')?;
	let Some(sign) = value[time_start..]
		.rfind(['+', '-'])
		.map(|i| i + time_start)
	else {
		return Some((value, 0));
	};
	let (head, zone) = value.split_at(sign);
	let digits = &zone[1..];
